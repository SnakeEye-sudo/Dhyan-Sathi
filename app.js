...
(() => {
  ...
  `;
    ...
    function monthlyPraise(monthSeconds) {
    ...
    function renderHistory() {
      historyList.innerHTML = state.logs.length ? state.logs.slice(0, 6).map((log) => `
      ...
      `).join("") : '<article class="history-item"><strong>Abhi koi session save nahi hua</strong><p>Ek full round complete karte hi log ban jayega.</p></article>';
    }
    ...
    `;
      watchHint.textContent = state.running ? "Focus chal raha hai. Overlay watch active hai." : "Ready ho to focus start karo.";
